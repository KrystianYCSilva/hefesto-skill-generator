...
def load_context():
    ...
    # Basic context loading: list available templates and ensure metadata template exists
    templates_dir = Path("commands/templates")
    available = []
    if templates_dir.exists():
        for p in templates_dir.iterdir():
            # Detect directories containing SKILL.md.template
            if p.is_dir() and (p / "SKILL.md.template").exists():
                available.append(p.name)

    # Always include the fallback skill-template.md when present
    if (templates_dir / "skill-template.md").exists():
        available.append("base-fallback")

    # Expose as a global for other functions to use (non-ideal but simple)
    global AVAILABLE_TEMPLATES
    AVAILABLE_TEMPLATES = list(dict.fromkeys(available))

    # Ensure metadata template presence warning
    if not (templates_dir / "metadata-template.yaml").exists():
        print("[WARN] metadata-template.yaml not found in commands/templates/")
...
def parse_arguments(args):
    ...
    parser.add_argument(
        "description", nargs="?", help="Natural language description of skill to create"
    )
    parser.add_argument(
        "--target",
        dest="target_cli",
        help="Target specific CLI (default: all detected)",
    )
    parser.add_argument(
        "--template",
        dest="template_name",
        default="base",
        help="Use specific template variant (default: base)",
    )
...
def detect_installed_clis():
    ...
    # Also probe system PATH for common CLI binaries as a best-effort
    try:
        import shutil
        ...
        for cid, binname in cli_binaries.items():
            if shutil.which(binname) and cid not in detected_clis:
                detected_clis.append(cid)
    except Exception:
        # If probing PATH fails, ignore — we still return detected from MEMORY.md
        pass

    return list(dict.fromkeys(detected_clis))
...
def detect_collision(skill_name, targets):
    """Detect if skill already exists. Returns (ok: bool, skill_name: str)."""
    collisions = []
    for cli in targets:
    ...
    if collisions:
        result = handle_collision(skill_name, collisions)
        # handle_collision may return:
        # - False => user cancelled / abort
        # - True  => user chose overwrite (ok, keep same name)
        # - str   => new sanitized name (ok, proceed)
        if result is False:
    ...
    return True, skill_name  # No collision
...
def handle_collision(skill_name, collisions):
    ...
    if action == "cancel":
        ...
        # Loop until a non-colliding name is provided or user cancels
        while True:
            ...
            # Check collisions for the new name
            still_collide = []
            for path in collisions:
                # Re-map existing collision paths to target directories and check
                for cli in [
                    p
                    for p in [
                        c.strip(".")
                        for c in set([Path(p).parts[0] for p in collisions])
                    ]
                    if p
                ]:
                    check_path = Path(f".{cli}/skills/{new_name}/SKILL.md")
                    if check_path.exists():
                        still_collide.append(str(check_path))

            if still_collide:
                print(f"Name '{new_name}' still conflicts: {still_collide}")
                try_again = input("Try another name? [y/N]:\n> ").strip().lower()
                if try_again != "y":
                    return False
                else:
                    continue
            # No collision with new name
            print(f"New name selected: {new_name}")
            return new_name
    elif action == "overwrite":
        # Create backup (best-effort print; actual backup not implemented here)
        print(
            f"Backup created: .hefesto/backups/{skill_name}-{datetime.now().strftime('%Y%m%d_%H%M%S')}.tar.gz"
        )
        return True
    else:
...
def generate_skill_content(template, skill_data):
    ...
    content = content.replace(
        "{{USE_WHEN}}", f"when you need to {skill_data['description']}"
    )
    content = content.replace(
        "{{SKILL_BODY}}", f"This skill helps with: {skill_data['description']}"
    )
...
def generate_metadata(skill_data):
    ...
    template = template.replace(
        "{{PLATFORMS}}", ", ".join([f'"{cli}"' for cli in skill_data["targets"]])
    )
    template = template.replace("{{DEPENDENCIES}}", "")
    template = template.replace(
        "{{EXAMPLE_PROMPT}}",
        f"Use the {skill_data['name']} skill to {skill_data['description']}",
    )
...
def validate_skill(content):
    ...
    if "name" not in data:
        ...
        errors.append(
            f"Invalid name '{data['name']}'. Must be lowercase, alphanumeric, with hyphens."
        )
    ...
    if line_count > 500:
        errors.append(
            f"SKILL.md exceeds 500 lines ({line_count}). Move content to references/."
        )
...
def human_gate(skill_name, description, targets, skill_content, validation_result):
    ...
    if action == "approve":
        ...
        print("-" * 40)
        return human_gate(
            skill_name, description, targets, skill_content, validation_result
        )
    elif action == "edit":
...
if __name__ == "__main__":
    ...
    if "--kotlin" in args:
        ...
        args.remove("--kotlin")
        args = [
            "fundamentals of kotlin programming including coroutines, compose, and modern features"
        ] + args
