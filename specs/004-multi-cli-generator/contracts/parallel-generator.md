# Contract: Parallel Generator

**Feature**: 004-multi-cli-generator  
**Component**: Multi-CLI Parallel Execution System  
...
The Parallel Generator orchestrates simultaneous skill generation for multiple AI CLIs, managing concurrent execution, progress tracking, error handling, and atomic rollback on failure.
...
```yaml
generation_request:
  skill_content: SkillContent     # Generic skill content (pre-Human Gate approval)
  target_clis: list[string]       # CLI names from Detection Report
  execution_mode: enum            # parallel | sequential
  temp_directory: string          # Temporary staging directory
```
...
**Output Type**: Generation Report (see data-model.md §5)
...
**Description**: Generates skill for all target CLIs in parallel (or sequential based on mode).
...
**Error Handling**: Immediate rollback on first failure, no partial states

---

### `generate_single_cli(task: GenerationTask, adapter: CLIAdapter, temp_dir: string) -> GenerationTask`

**Description**: Generates skill for a single CLI in temp directory.
...
**Error Handling**: Failures isolated to task - don't affect sibling tasks until merge phase

---

### `validate_all_generated(tasks: list[GenerationTask], temp_dir: string) -> boolean`

**Description**: Validates all generated skills before committing to target directories.
...
**Description**: Atomically moves validated skills from temp directory to target CLI directories.
...
**Error Handling**: Filesystem errors (permissions, disk full) trigger immediate rollback

---

### `rollback_all(tasks: list[GenerationTask], temp_dir: string) -> void`

**Description**: Cleans up temporary directory and marks all tasks as rolled back.
...
**Failure Handling**: If temp directory deletion fails, log warning but continue (cleanup on next run)
...
1. **Parallel Generation Success (3 CLIs)**
   - **Setup**: Detect claude, gemini, opencode
   - **Execute**: generate_all() in parallel mode
   - **Expected**: All 3 succeed, 3x faster than sequential, files in all 3 directories

2. **Sequential Generation Success (3 CLIs)**
   - **Setup**: Detect claude, gemini, opencode
   - **Execute**: generate_all() in sequential mode
   - **Expected**: All 3 succeed, baseline performance

3. **Parallel vs Sequential Performance**
   - **Setup**: Detect 5 CLIs
   - **Measure**: Parallel time vs sequential time
   - **Expected**: Parallel < 0.33 × sequential (3x faster)

4. **Validation Failure Triggers Rollback**
   - **Setup**: Inject validation failure in gemini adapter
   - **Execute**: generate_all() for 3 CLIs
   - **Expected**: All rolled back, error message points to gemini failure

5. **Permission Error Triggers Rollback**
   - **Setup**: Remove write permission on one CLI directory
   - **Execute**: generate_all() for 3 CLIs
   - **Expected**: All rolled back, error message describes permission issue

6. **User Cancellation (SIGINT)**
   - **Setup**: Start generation, send SIGINT during execution
   - **Expected**: Immediate termination, temp directory cleaned up

7. **Idempotence Check Integration**
   - **Setup**: Skill already exists in one CLI directory
   - **Execute**: generate_all() for 3 CLIs
   - **Expected**: Prompt for action, respect user choice
...
### Internal
- CLI Detector (provides target CLI list)
- CLI Adapter (applies transformations)
- Rollback Handler (cleanup logic)
- Existing `/hefesto.validate` command
...
- [CLI Detector](./cli-detector.md) - Provides target CLI list
- [CLI Adapter](./cli-adapter.md) - Transforms skills per CLI
- [Rollback Handler](./rollback-handler.md) - Atomic cleanup logic
