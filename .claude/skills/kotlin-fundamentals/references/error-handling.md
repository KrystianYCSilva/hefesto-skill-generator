...
fun main() = runBlocking {
    ...
    // Falha neste coroutine NÃO cancela o outro
    scope.launch {
        throw RuntimeException("Job 1 falhou")
    }
...
fun main() = runBlocking {
    ...
    try {
        val resultado = deferred.await()  // exceção é lançada aqui
    } catch (e: IllegalStateException) {
        println("Capturado no await: ${e.message}")
    }
}
```
